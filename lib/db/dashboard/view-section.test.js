...
describe('Dashboard.View methodes are working', () => {
    ...
    it('Dashboard.viewSection.add()', async () => {
        ...
        const id = await database.dashboard.viewSection.add('Test', 'Test', viewId, "0,0,1,1", 1, 0)
        ...
        expect(row).toHaveProperty('zoom', 1)
        expect(row).toHaveProperty('reload_interval', 0)
    ...
    it('Dashboard.viewsection.update()', async () => {
        ...
        let viewSection = await database.db.get(`SELECT * FROM viewSection WHERE id=${viewSectionId}`)
        ...
        expect(viewSection).toHaveProperty('zoom')
        await database.dashboard.viewSection.update(viewSectionId, false, false, false, false, 4)
        viewSection = await database.db.get(`SELECT * FROM viewSection WHERE id=${viewSectionId}`)
        expect(viewSection.zoom).toBe(4)
        expect(viewSection).toHaveProperty('reload_interval')
        await database.dashboard.viewSection.update(viewSectionId, false, false, false, false, false, 100)
        viewSection = await database.db.get(`SELECT * FROM viewSection WHERE id=${viewSectionId}`)
        expect(viewSection.reload_interval).toBe(100)
